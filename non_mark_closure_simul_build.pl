...
my @FILES = qw/
    self_library.f90
    steady_state_slab_non_mark_closure_simul.f90
/;
my $EXEC_NAME = "steady_state_slab_non_mark_closure_simul.exe";
my $OPT_LEVEL = 3;
...
sub main {
    ...
    # Compile executable
    comp("$COMPILER -O$OPT_LEVEL -o $BIN/$EXEC_NAME" . $obj_string . " -llapack");
...
sub clean_data {
    ...
    while (my $filename = readdir($dir)) {
        unlink "$OUT/$filename" if $filename =~ /non_mark_closure/;
    }
    closedir $dir;
